...
//            var wasShown = $ax.visibility.IsIdVisible(elementId);
    ...
    var _move = function (elementId, x, y, options, moveTo) {
        ...
        if ($ax.public.fn.IsLayer(obj.type)) {
            ...
            $ax.move.MoveWidget(elementId + '_container', x, y, options, moveTo,
                function () {
                ...
                if ($ax.public.fn.isCompoundVectorHtml(query[0])) {
                    ...
                    children.each(function(i, child) {
                        ...
                        var stretchMatrix = function (widthRatio, heightRatio) {
                            ...
                            var xShift = (widthRatio - 1.0) * centerDisplacement.x;
                            var yShift = (heightRatio - 1.0) * centerDisplacement.y;
                    ...
                    children.each(function (i, child) {
                        ...
                        var isSketch = child.id.indexOf("_image_sketch", this.length - "_image_sketch".length) !== -1;
                        if((isSketch || isConnector) && child.tagName == 'IMG') {
    ...
    var _getLoc = function(id, vert, high, relative) {
        ...
        if(fixed.valid) loc = fixed.offset;
            ...
            for(var i = 0; i < parents.length; i++) {
                ...
                if($ax.public.fn.IsLayer($ax.getTypeFromElementId(parentId))) parent = $jobj(parentId + '_container');
